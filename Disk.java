...
public class Disk 
{
	...
	public Disk()															//Default ctor
	{
		...
		store = new char[this.sectorCount][this.sectorSize];					//Initialize Disk spaces
	}
	...
	public void readSector(int sectorNumber, char[] buffer){				//Sector to buffer
		for(int i = 0; i < buffer.length; i++)									//Copy the contents of the sector whose
			buffer[i] = store[sectorNumber][i];									//number is the first parameter to the
	}																			//character array which is the second argument

	public void writeSector(int sectorNumber, char[] buffer){					//Buffer to sector
		for(int i = 0; i < buffer.length; i++)								//Copy the contents of disk buffer		
			store[sectorNumber][i] = buffer[i];								//to the sector whose number is the
	}																		//first argument
